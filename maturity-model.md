...
The team uses ODPM language: ontology, entities, relationships, actions, states, rules. The vocabulary is shared even if the model isn't written yet.

**You're here when:** people say "that's a new entity" or "which state is it in?" without being prompted.

**To advance:** write the vocabulary down. A [glossary](glossary.md) is enough to start.

## Level 2 — Documented

A lightweight ontology exists as an artifact: a glossary, a relationship map, and the actions, states, and rules. Not a heavy model — a page the team can point to.
...
Ontology changes are proposed, reviewed, and agreed — like pull requests. No single person redefines a shared entity alone.
...
The ontology becomes executable or semi-executable through tools, workflows, agents, or systems. Applications query the same objects, follow the same relationships, respect the same rules — there is no per-application re-interpretation of what an entity means.
