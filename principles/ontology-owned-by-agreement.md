# Ontology Is Owned by Agreement

No single person owns the ontology.

The ontology is owned by the team's agreement.

## Why This Matters

If the ontology can be changed by authority rather than agreement, it becomes a top-down artifact.

Teams receive it but do not share it.

A shared ontology requires the process of sharing, not just the result.

## How Agreement Works in Practice

A proposed change to the ontology is a Pull Request.

Review is where the ontology is tested.

Merge is where the team confirms the ontology is aligned enough to move forward.

This is not bureaucracy. It is the minimum structure that makes "shared" mean something.

## What This Prevents

- One team silently redefining an entity that other teams depend on.
- A sponsor overwriting the ontology without the implementation team's understanding.
- An ontology that exists on paper but not in practice.

## The Boundary

Agreement does not mean consensus on every word.

It means Decision Readiness: the ontology is aligned enough to move forward responsibly.

If two people disagree about an entity, that disagreement is an open question — not a blocker, but a thread to pull.

---

See also: [GitHub as an ODPM Tool](github-as-odpm-tool.md)
