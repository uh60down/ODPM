...
**Issue = Open Question**
In ODPM, we treat an Issue as an Open Question before it becomes a task.
It stays open until the question is resolved.
The thread is where understanding grows.

**Discussion = Chase Understanding**
Discussions are not decisions.
...
| Label | Meaning |
|-------|---------|
| `open-question` | Active conversation, idea not yet solid |
| `chase` | Understanding is being pursued |
| `proposed` | A refinement has been suggested |
| `landed` | Understanding crystallized into an artifact |
| `archived` | Question closed without resolution |

---

## How to Use This

When an idea is not solid — open an Issue.
Let the conversation grow in the thread.
Do not rush to close it.

When the idea crystallizes — commit it.
...
It turns out that collaborative software development
and collaborative understanding-building
